var PRODUCT_NAMES = [
  ...
  class FakeServer {
      ...
      for (var i = 0; i < PRODUCT_NAMES.length; i++) {
        ...
        for (var j = 0; j < PORTFOLIO_NAMES.length; j++) {
          ...
          for (var k = 0; k < 5; k++) {
            ...
            for (var l = 0; l < 5; l++) {
              var trade = this.createTradeRecord(
                productId,
                productName,
                portfolioId,
                portfolioName,
    ...
    getData(request, parentData, callback) {
      ...
      function getDataFromServer() {
        ...
        switch (groupKeys.length) {
          ...
          case 2:
            var books =
              that.productsMap[productId].childrenMap[portfolioId].children;
            result = books.map(that.mapBook.bind(this));
            break;
          case 3:
            var trades =
              that.productsMap[productId].childrenMap[portfolioId].childrenMap[
                bookId
              ].children;
    ...
    doBatch() {
      ...
      var book1 =
        portfolio.children[Math.floor(Math.random() * portfolio.children.length)];
      var book2 =
        portfolio.children[Math.floor(Math.random() * portfolio.children.length)];
    ...
    createOneTrade(transactions, product, portfolio, book) {
      var newTrade = this.createTradeRecord(
        product.productId,
        product.productName,
